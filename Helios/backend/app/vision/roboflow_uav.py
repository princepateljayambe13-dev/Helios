"""Roboflow serverless UAV detector adapter.

The API secret is obtained only from the deployment environment.  Results are
normalized before entering HELIOS tracking, events, and alerts.
"""
...
class RoboflowUavInferenceManager:
    def __init__(self, service: HeliosService, settings: Settings, camera_hub: CameraStreamHub) -> None:
        ...
        self.configs = [
            ...
            if m.get("enabled") and (m.get("mode") == "roboflow-uav" or "UAV" in m.get("classes", []))
        ]
    ...
    async def start(self) -> None:
        ...
        for config in self.configs:
            ...
            for camera in cameras:
                self.tasks.append(asyncio.create_task(self._run(camera["camera_id"], config), name=f"uav:{camera['camera_id']}"))
    ...
    async def _run(self, camera_id: str, config: dict[str, Any]) -> None:
        ...
        try:
            while not self.stopping:
                ...
                frame = await self.camera_hub.next_frame(camera_id, sequence); sequence = frame.sequence
        ...
        except Exception: LOGGER.exception("Roboflow UAV inference failed for %s", camera_id)
    ...
    def _detect(self, image: Any, camera_id: str, config: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if config.get("workflow_id"):
                workflow_output = client.run_workflow(
                    ...
                    parameters=config.get("parameters", {"classes": "drone"}),
                    use_cache=True,
                )
                ...
                result = client.infer(str(path), model_id=config.get("model_id", "uav-v1djs/3"))
        ...
        for prediction in predictions:
            ...
            observations.append(normalized_observation(camera_id=camera_id,object_type="UAV",confidence=confidence,xyxy=[x1,y1,x1+box_width,y1+box_height],image_width=width,image_height=height,model_name=config["name"],model_version=config.get("version"),attributes={"source_class":str(prediction.get("class", "uav")).lower()}))
        return observations
